select
CONCAT_WS('-','S', NATUREZAOPERACAO, replace(i.CFOP,'.',''), 'RJ', SIGLAUFDESTREM, replace(i.CLASSIFICACAOFISCAL,'.','')) as 'codigo'
, replace(i.CLASSIFICACAOFISCAL,'.','') as 'NCM(ip)'
, NATUREZAOPERACAO as 'NaturezaOperacao(d)'
, replace(i.CFOP,'.','') as 'CdCfop(i)'
, 'RJ' as 'Uf(pe)'
, SIGLAUFDESTREM as 'Uf(pd)'
...
from dofit001_taxweb_s d, ldfit001_taxweb_s i
